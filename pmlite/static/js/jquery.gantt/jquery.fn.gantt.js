...
(function ($, undefined) {
    ...
    // 自定义选择器“：findday”用于在毫秒内的指定日期进行匹配。
    // 向选择器传递一个以毫秒为单位的日期，如果元素日期匹配，则将元素添加到选择过滤器中，如包含可解析日期的id属性所确定。
    ...
    // 自定义选择器“：findweek”用于在ms中的指定周进行匹配。
        ...
        var core = {
            ...
            elementFromPoint: (function(){ // IIFE
                ...
                if (document.compatMode === "CSS1Compat") {
                    return function (x, y) {
                        // 相对视口（viewport）的水平滚动距离
                        x -= window.pageXOffset;
                        // 相对视口（viewport）的竖直滚动距离
                        y -= window.pageYOffset;
                        return document.elementFromPoint(x, y);  // 返回位于屏幕坐标（x,y）处的最顶层元素
                    };
                }
            ...
            // 在这里，我们计算数据准备好后的行数、页数以及可见的开始日期和结束日期
            init: function (element) {
                element.rowsNum = element.data.length;  // 总行数
                element.pageCount = Math.ceil(element.rowsNum / settings.itemsPerPage);  //总页数。 Math.ceil()：为向上取整数
                element.rowsOnLastPage = element.rowsNum - (Math.floor(element.rowsNum / settings.itemsPerPage) * settings.itemsPerPage);
                // 最后一页的行数。 Math.floor():向下取整数

                element.dateStart = tools.getMinDate(element);  // 最小日期
                element.dateEnd = tools.getMaxDate(element);  // 最大日期
            ...
            render: function (element) {
                ...
                core.fillData(element, $dataPanel, $leftPanel);  // 填充进度条
                core.fillData2(element, $dataPanel, $leftPanel);  // 填充进度条（实际）
            ...
            rightPanel: function (element, leftPanel /* <- never used? */) {
                ...
                // 按'日'显示时，工作日、周六、周日有对应的类已显示不同的颜色：wd(工作日),sa(周六),sn(周日)
                ...
                default:
                    ...
                    range = tools.parseDateRange(element.dateStart, element.dateEnd);  // 根据最小/最大日期创建Date对象range
                    dataPanelWidth = range.length * tools.getCellSize();  // 获取最大宽度（时间个数x24）

                    year = range[0].getFullYear();  // 第一个日期的 年
                    month = range[0].getMonth();  // 第1个日期的 月

                    for (i = 0, len = range.length; i < len; i++) {  // 遍历每一个日期
                        rday = range[i];
            ...
            createProgressBar: function (label, desc, classNames, dataObj) {
                ...
                if (desc) {
                    bar
                      .mouseenter(function (e) {  // 鼠标进入元素时触发
                      ...
                      .mouseleave(function () {  // 鼠标离开元素时触发
                          $(".fn-gantt-hint").remove();
                      })
                      .mousemove(function (e) {  // 元素上鼠标移动事件
...
//                            }
        ...
        this.each(function () {
            // this指jQuery对象 div.gantt(<div class="gantt"></div>)
            this.data = null;        // Received data  接收数据
            this.pageNum = 0;        // Current page number  当前页码
            this.pageCount = 0;      // Available pages count  可用页数
            this.rowsOnLastPage = 0; // How many rows on last page  最后一页有多少行
            this.rowsNum = 0;        // Number of total rows  总行数
            this.hPosition = 0;      // Current position on diagram (Horizontal)  图表上的当前位置（水平）
            this.dateStart = null;
            this.dateEnd = null;
